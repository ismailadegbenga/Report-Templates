---
title: "Ecto — Phoenix v1.8.11"
source: "https://phoenix.hexdocs.pm/1.8.11/ecto.html"
author:
published:
created: 2026-08-17
description:
...
> 
> - Gone through the [introductory guides](https://phoenix.hexdocs.pm/1.8.11/installation.html)
> - Got a Phoenix application [up and running](https://phoenix.hexdocs.pm/1.8.11/up_and_running.html)

Most web applications today need some form of data validation and persistence. In the Elixir ecosystem, we have [`Ecto`](https://hexdocs.pm/ecto/3.14.1/Ecto.html) to enable this. Before we jump into building database-backed web features, we're going to focus on the finer details of Ecto to give a solid base to build our web features on top of. Let's get started!
...
This guide assumes that we have generated our new application with Ecto integration and that we will be using PostgreSQL. The introductory guides cover how to get your first application up and running. For using other databases, see the [Swapping Databases](https://phoenix.hexdocs.pm/1.8.11/swapping_databases.html) how-to guide.
...
Our repo has three main tasks - to bring in all the common query functions from \[[`Ecto.Repo`](https://hexdocs.pm/ecto/3.14.1/Ecto.Repo.html)\], to set the `otp_app` name equal to our application name, and to configure our database adapter. We'll talk more about how to use `Hello.Repo` in a bit.
...
Right now, we have two transformations in our pipeline. In the first call, we invoke [`Ecto.Changeset.cast/3`](https://hexdocs.pm/ecto/3.14.1/Ecto.Changeset.html#cast/3), passing in our external parameters and marking which fields are required for validation.

[`cast/3`](https://hexdocs.pm/ecto/3.14.1/Ecto.Changeset.html#cast/3) first takes a struct, then the parameters (the proposed updates), and then the final field is the list of columns to be updated. [`cast/3`](https://hexdocs.pm/ecto/3.14.1/Ecto.Changeset.html#cast/3) also will only take fields that exist in the schema.

Next, [`Ecto.Changeset.validate_required/3`](https://hexdocs.pm/ecto/3.14.1/Ecto.Changeset.html#validate_required/3) checks that this list of fields is present in the changeset that [`cast/3`](https://hexdocs.pm/ecto/3.14.1/Ecto.Changeset.html#cast/3) returns. By default with the generator, all fields are required.
...
Let's say we want to perform at least some rudimentary format validation on the `email` field. All we want to check for is the presence of the `@`. The [`Ecto.Changeset.validate_format/3`](https://hexdocs.pm/ecto/3.14.1/Ecto.Changeset.html#validate_format/3) function is just what we need.
...
First, we imported [`Ecto.Query`](https://hexdocs.pm/ecto/3.14.1/Ecto.Query.html), which imports the [`from/2`](https://hexdocs.pm/ecto/3.14.1/Ecto.Query.html#from/2) macro of Ecto's Query DSL. Next, we built a query which selects all the email addresses in our users table. Let's try another example.
...
In addition to inserts, we can also perform updates and deletes with [`Repo.update/2`](https://hexdocs.pm/ecto/3.14.1/Ecto.Repo.html#c:update/2) and [`Repo.delete/2`](https://hexdocs.pm/ecto/3.14.1/Ecto.Repo.html#c:delete/2) to update or delete a single schema. Ecto also supports bulk persistence with the [`Repo.insert_all/3`](https://hexdocs.pm/ecto/3.14.1/Ecto.Repo.html#c:insert_all/3), [`Repo.update_all/3`](https://hexdocs.pm/ecto/3.14.1/Ecto.Repo.html#c:update_all/3), and [`Repo.delete_all/2`](https://hexdocs.pm/ecto/3.14.1/Ecto.Repo.html#c:delete_all/2) functions.
...
Migrations are a programmatic, repeatable way to affect changes to a database schema. Phoenix generators take care of generating migrations for us whenever we create a new context or schema, but if you want to generate a migration from scratch, [`mix ecto.gen.migration`](https://hexdocs.pm/ecto_sql/3.14.0/Mix.Tasks.Ecto.Gen.Migration.html) has our back. Let's see an example.
...
For more information on how to modify your database schema please refer to the [Ecto's migration DSL docs](https://ecto-sql.hexdocs.pm/Ecto.Migration.html). For example, to alter an existing schema see the documentation on Ecto’s [`alter/2`](https://hexdocs.pm/ecto_sql/3.14.0/Ecto.Migration.html#alter/2) function.

That's it! We're ready to run our migration.

### mix ecto.migrate

Once we have our migration module ready, we can simply run [`mix ecto.migrate`](https://hexdocs.pm/ecto_sql/3.14.0/Mix.Tasks.Ecto.Migrate.html) to have our changes applied to the database. We have already used it earlier in this chapter, but let's take it for a spin once more for our newly generated migration.
...
When we roll back a migration, [`mix ecto.rollback`](https://hexdocs.pm/ecto_sql/3.14.0/Mix.Tasks.Ecto.Rollback.html), to be discussed next, we will remove the record representing this migration from `schema_migrations`.
...
The [`mix ecto.rollback`](https://hexdocs.pm/ecto_sql/3.14.0/Mix.Tasks.Ecto.Rollback.html) task will reverse the last migration we have run, undoing the schema changes. `ecto.migrate` and `ecto.rollback` are mirror images of each other.
